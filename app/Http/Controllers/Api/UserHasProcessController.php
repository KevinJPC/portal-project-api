...
class UserHasProcessController extends Controller
{
    ...
    public function getUserProcesses(){
        try {
            ...
            ->select('processes.name', 'processes.created_at', 'users_has_processes.activity',
                'users_has_processes.status')
            ->join('users_has_processes', function ($join) {
                $join->on('processes.id', '=', 'users_has_processes.process_id')
                    ->where('users_has_processes.user_id', '=', Auth::user()->id);
                })            
            ->paginate(10);
